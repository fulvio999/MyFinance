...
//------------------------------------ CREATE QUERY -----------------------------
    ...
    */
    function updateExpense(expenseId, id_subcategory_current_new, amount, date, note) {

        console.log('Updating expenseId:'+expenseId+', id_subcategory:'+id_subcategory_current_new+', with new amount: '+amount+', new date: '+date+', new note: '+note);
        ...
        db.transaction(function(tx) {
            var rs = tx.executeSql('UPDATE expense SET id_subcategory=?, amount=?, date=?, note=? WHERE id=? ;', [id_subcategory_current_new, Number(amount), expenseDateFormatted, note, expenseId ]);
            if (rs.rowsAffected > 0) {
                res = "OK";
            } else {
                res = "Error";
    ...
    /* Get an expense given a categoryId and subCategoryId in argument  */
    function getExpsenseByCategoryAndSubCategory(categoryId,subCategoryId){

        var db = getDatabase();
        var res = "";

        db.transaction(function(tx) {
              res = tx.executeSql('SELECT * FROM expense where id_subcategory='+subCategoryId+' and id_category='+categoryId);
          }
        );
